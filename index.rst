Truly Understanding the Teachings of the Buddha
===============================================

.. role:: pdfpage

.. admonition:: Editorial note

   This document is available as :ebookweb:`web (HTML) <html/>`, :ebookweb:`print (PDF) <latex/markwell-truly-understanding.pdf>` and :ebookweb:`e-book (ePub) <epub/markwell-truly-understanding.epub>`.

   It was prepared from the `original PDF <https://6db146.a2cdn1.secureserver.net/wp-content/uploads/2019/04/TrulyUnderstandingtheTeachingsoftheBuddha.pdf>`__ from 2019 (linked from `anthonymarkwell.com <https://anthonymarkwell.com>`__), following encouragement by the author (indicated page numbers refer to the 2019 PDF). There was no alteration to the content. Sectioning hierarchy was slightly modified for easier navigation.

   Created `from the source <https://github.com/edhamma/markwell-truly/>`__, git version |release|, on |today|.


.. admonition:: About the book
   :class: note

   :pdfpage:`306` This book contains the Dhamma talks given by Anthony Markwell, when he was resident teacher at Wat Kow Tahm Insight Meditation Center in Thailand during Vipassana meditation retreats offered by him.

   Anthony is teaching the Dhamma in a unique way, comprehensive and interlinked, which allows the reader to get a profound understanding of the teachings of the Buddha.

   This book is not only a comprehensive guide to understanding Vipassana meditation but it gives the full background and practical instructions and maps out the way leading to Nirvana – here and now in this very life.



.. toctree::
   :numbered:

   0-a.md
   0-b.md
   1-a.md
   1-b.md
   2-a.md
   2-b.md
   3-a.md
   3-b.md
   4-a.md
   4-b.md
   5-a.md
   5-b.md
   6-a.md
   6-b.md
   7-a.md
   7-b.md
   final.md
   chanting.md
   appendices.rst
   author.md
